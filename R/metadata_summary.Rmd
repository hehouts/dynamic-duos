...
### Chunk 5 - Finding existing filenames for viromes in the ibdmdb metadata
...
### Chunk 4 - s5 subset: File IDs for viromes
...
```{r}
unzipping %>% filter(zip_type == "VIR") %>% select(file) %>%  
  write_csv("../snakecrate/data/raw_data/vir/s5_vir_prefixes.csv", col_names = F)
...
## Makes the filename.txt's for all the subtypes
...
mutate(uid = case_when(
  ...
  filter(zip_type== "P") %>% 
  select(file)%>%  
  write_csv("../snakecrate/data/raw_data/p/all.csv", col_names = F)
